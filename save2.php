    ...
    if($money <= 1000){
        if($money_diff >= 0){
            ...
            if ($conn->query($sql) === TRUE) {
            	...
            	$sql = "UPDATE common_wallet SET c_money = c_money + $money WHERE manager_id = (SELECT Manager_id FROM wallet_relation where uid = (SELECT ID FROM member where E_mail = '$_SESSION[E_mail]')) ";
            	
            	if ($conn->query($sql) === TRUE) {
            		$conn->close();
            		include("login2.php");
            	
            		$sql = "UPDATE wallet_relation SET money = money + $money WHERE uid = (SELECT ID FROM member where E_mail = '$_SESSION[E_mail]') and yes_no = '1'";
...
<form style = "font-size: 200%; text-align:center;" action="/save2.php" method="post">
  <br>
  ...
  <?php if($money_err2 == 0){
      echo "<span style = 'color:#FF0000;font-size:100%;'>個人錢包餘額不足</span><br />";
  }?>
  <br>
